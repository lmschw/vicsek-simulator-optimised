...
# Evaluates the data produced by neighbour_selection_data_random_subset.py (the variant where the
# local order used for the switching decision is computed from a random subset of x neighbours,
# redrawn every timestep, instead of from all in-radius neighbours). Produces the same three plots
# as neighbour_selection_data_plots.py (order over time, switch value percentage over time, number
# of clusters over time), covering only the four switching combinations this variant exists for,
# swept over x = 1..5.

BASE_DATA_LOCATION = "/media/lilly/WD Elements/data_july26/"
OUTPUT_ROOT = "plots/neighbour_selection_data_random_subset"
                ...
                for subsetSize in subsetSizes:
                    for thresholds in switchThresholdOptions:
                        for eventEffect in eventEffects:
                            base = f"local_ksw_1ev_randsub_x={subsetSize}_{{}}_st={{}}_d={density}_n={n}_r={radius}_nsm={nsm.value}_kCombo={kCombo[0]}-{kCombo[1]}_th={thresholds}_ee={eventEffect.val}"
                            yield dict(
                                comboType="local_ksw_1ev_randsub",
                                ...
                                paramSuffix=f"x={subsetSize}_d={density}_n={n}_r={radius}_nsm={nsm.value}_kCombo={kCombo[0]}-{kCombo[1]}_th={thresholds}_ee={eventEffect.val}",
            ...
            for nsmCombo in nsmCombos:
                for k in ks:
                    for subsetSize in subsetSizes:
                        for thresholds in switchThresholdOptions:
                            for eventEffect in eventEffects:
                                base = (f"local_nsmsw_1ev_randsub_x={subsetSize}_{{}}_st={{}}_d={density}_n={n}_r={radius}"
                                        f"_nsmCombo={nsmCombo[0].value}-{nsmCombo[1].value}_k={k}_th={thresholds}_ee={eventEffect.val}")
                                yield dict(
                                    comboType="local_nsmsw_1ev_randsub",
                                    ...
                                    paramSuffix=f"x={subsetSize}_d={density}_n={n}_r={radius}_nsmCombo={nsmCombo[0].value}-{nsmCombo[1].value}_k={k}_th={thresholds}_ee={eventEffect.val}",
                                    evalInterval=LOCAL_EVAL_INTERVAL,
                                    eventShading=eventWindow,
    ...
    for density in densities:
        ...
        for radius in radii:
            for nsm in reducedNeighbourSelectionMechanisms:
                for subsetSize in subsetSizes:
                    for thresholds in switchThresholdOptions:
                        base = f"local_ksw_noev_randsub_x={subsetSize}_{{}}_st={{}}_d={density}_n={n}_r={radius}_nsm={nsm.value}_kCombo={kCombo[0]}-{kCombo[1]}_th={thresholds}"
                        yield dict(
                            comboType="local_ksw_noev_randsub",
                            basePathOrdered=f"{BASE_DATA_LOCATION}{base.format('ordered', kCombo[0])}",
                            basePathRandom=f"{BASE_DATA_LOCATION}{base.format('random', kCombo[1])}",
                            paramSuffix=f"x={subsetSize}_d={density}_n={n}_r={radius}_nsm={nsm.value}_kCombo={kCombo[0]}-{kCombo[1]}_th={thresholds}",
                            evalInterval=LOCAL_EVAL_INTERVAL,
                            eventShading=None,
                            switchType=SwitchType.K,
                            switchTypeOptions=(kCombo[0], kCombo[1]),
        ...
        for radius in radii:
            for nsmCombo in nsmCombos:
                for k in ks:
                    for subsetSize in subsetSizes:
                        for thresholds in switchThresholdOptions:
                            base = (f"local_nsmsw_noev_randsub_x={subsetSize}_{{}}_st={{}}_d={density}_n={n}_r={radius}"
                                    f"_nsmCombo={nsmCombo[0].value}-{nsmCombo[1].value}_k={k}_th={thresholds}")
                            yield dict(
                                comboType="local_nsmsw_noev_randsub",
                                basePathOrdered=f"{BASE_DATA_LOCATION}{base.format('ordered', nsmCombo[0].value)}",
                                basePathRandom=f"{BASE_DATA_LOCATION}{base.format('random', nsmCombo[1].value)}",
                                paramSuffix=f"x={subsetSize}_d={density}_n={n}_r={radius}_nsmCombo={nsmCombo[0].value}-{nsmCombo[1].value}_k={k}_th={thresholds}",
                                evalInterval=LOCAL_EVAL_INTERVAL,
                                eventShading=None,
                                switchType=SwitchType.NEIGHBOUR_SELECTION_MECHANISM,
                                switchTypeOptions=(nsmCombo[0], nsmCombo[1]),
