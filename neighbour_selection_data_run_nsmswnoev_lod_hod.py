                ...
                for thresholds in threshold_options:
                    ...
                    for startingCondition in ["ordered", "random"]:
                        ...
                        for i in range(iStart, iStop):
                            ...
                            iEnd = time.time()
                            ServiceGeneral.logWithTime(f"Completed NSMSW NOEV d={density}, r={radius}, nsm={nsmCombo[0].value}-{nsmCombo[1].value}, k={k}, thresholds={thresholds}, st={startingCondition}, i={i} in {ServiceGeneral.formatTime(iEnd-iStartTime)}")
                        stEnd = time.time()
                        ServiceGeneral.logWithTime(f"Completed NSMSW NOEV d={density}, r={radius}, nsm={nsmCombo[0].value}-{nsmCombo[1].value}, k={k}, thresholds={thresholds}, st={startingCondition} in {ServiceGeneral.formatTime(stEnd-stStart)}")
                    kEnd = time.time()
                    ServiceGeneral.logWithTime(f"Completed NSMSW NOEV d={density}, r={radius}, nsm={nsmCombo[0].value}-{nsmCombo[1].value}, k={k}, thresholds={thresholds} in {ServiceGeneral.formatTime(kEnd-kStart)}")
                kEnd = time.time()
