...
densities = [0.05, 0.06, 0.07, 0.08, 0.09]
radii = [5, 10]
ks = [1, 5]
...
for domainSize in domainSizes:
                        ...
                        for startingCondition in ["ordered", "random"]:
                            ...
                            for i in range(iStart, iStop):
                                ...
                                ServiceSavedModel.saveModel(simulationData=simulationData, path=savePath, 
                                                            modelParams=simulator.getParameterSummary())
                                iEnd = time.time()
                                ServiceGeneral.logWithTime(f"Completed NSMSW d={density}, r={radius}, nsm={nsmCombo[0].value}-{nsmCombo[1].value}, k={k}, ee={eventEffect.val}, st={startingCondition}, i={i} in {ServiceGeneral.formatTime(iEnd-iStartTime)}")
                            stEnd = time.time()
                            ServiceGeneral.logWithTime(f"Completed NSMSW d={density}, r={radius}, nsm={nsmCombo[0].value}-{nsmCombo[1].value}, k={k}, ee={eventEffect.val}, st={startingCondition} in {ServiceGeneral.formatTime(stEnd-stStart)}")
                        eeEnd = time.time()
                        ServiceGeneral.logWithTime(f"Completed NSMSW d={density}, r={radius}, nsm={nsmCombo[0].value}-{nsmCombo[1].value}, k={k}, ee={eventEffect.val} in {ServiceGeneral.formatTime(eeEnd-eeStart)}")
                    kEnd = time.time()
    ...
    tendLocalKswNoEv = time.time()
    ServiceGeneral.logWithTime(f"completed k switching without event: {ServiceGeneral.formatTime(tendLocalKswNoEv-tstartLocalKswNoEv)}")
    
    # ----------------------------- LOCAL - nsm switching without event --------------------------------------------------------
    tstartLocalNsmswNoEv = time.time()
                    ...
                    for startingCondition in ["ordered", "random"]:
                        ...
                        for i in range(iStart, iStop):
                            ...
                            ServiceSavedModel.saveModel(simulationData=simulationData, path=savePath, 
                                                        modelParams=simulator.getParameterSummary())
                            iEnd = time.time()
                            ServiceGeneral.logWithTime(f"Completed NSMSW NOEV d={density}, r={radius}, nsm={nsmCombo[0].value}-{nsmCombo[1].value}, k={k}, st={startingCondition}, i={i} in {ServiceGeneral.formatTime(iEnd-iStartTime)}")
                        stEnd = time.time()
                        ServiceGeneral.logWithTime(f"Completed NSMSW NOEV d={density}, r={radius}, nsm={nsmCombo[0].value}-{nsmCombo[1].value}, k={k}, st={startingCondition} in {ServiceGeneral.formatTime(stEnd-stStart)}")
                    kEnd = time.time()
                    ServiceGeneral.logWithTime(f"Completed NSMSW NOEV d={density}, r={radius}, nsm={nsmCombo[0].value}-{nsmCombo[1].value}, k={k} in {ServiceGeneral.formatTime(kEnd-kStart)}")
                nsmEnd = time.time()
                ServiceGeneral.logWithTime(f"Completed NSMSW NOEV d={density}, r={radius}, nsm={nsmCombo[0].value}-{nsmCombo[1].value} in {ServiceGeneral.formatTime(nsmEnd-nsmStart)}")
            rEnd = time.time()
    ...
    tendLocalNsmswNoEv = time.time()
    ServiceGeneral.logWithTime(f"completed nsm switching without event: {ServiceGeneral.formatTime(tendLocalNsmswNoEv-tstartLocalNsmswNoEv)}")
    
    tendDs = time.time()
    ServiceGeneral.logWithTime(f"duration domainSize {domainSize}: {ServiceGeneral.formatTime(tendDs-tStartDS)}")
tend = time.time()
